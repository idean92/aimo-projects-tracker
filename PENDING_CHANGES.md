...
| # | Item | Status |
|---|------|--------|
| P21 | App retirement — the merged AIMO KSIA Tracker (`idean92/aimo-tracker`) is taking over this app's Cloudflare Worker/URL | **Drafted, 23 Aug 2026, not yet executed.** Owner decision: this app is retired outright once the cutover lands, not moved elsewhere. **Action item before that cutover can deploy: disconnect this repo's Cloudflare Git integration** (Settings → Build → disconnect) — it currently deploys on any-branch push (verified 06 Aug 2026), and once the merged app's `wrangler.toml` targets this Worker name, a stray push here would silently overwrite it. Dashboard-only; cannot be done or verified from an agent session. See `idean92/aimo-tracker`'s `PENDING_CHANGES.md` INF-10 and `docs/DEPLOYMENT.md` for the full runbook |
